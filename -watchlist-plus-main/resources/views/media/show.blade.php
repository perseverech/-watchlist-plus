...
<div style="max-width: 1000px; margin: 0 auto; padding: 48px 24px;">
    ...
    <div style="margin-top: 48px;">
        ...
        @forelse($reviews as $review)
            <div style="background: #13131a; border: 1px solid #1e1e2e; border-radius: 12px; padding: 18px; margin-bottom: 12px;">
                ...
                {{-- Delete Button: Only visible to the review author OR a Moderator/Admin --}}
                @if(auth()->check() && (auth()->id() == $review->user_id || auth()->user()->isModerator()))
                    <form method="POST" action="{{ route('reviews.destroy', $review->id) }}" style="margin-top: 12px; text-align: right;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" style="background: transparent; color: #7f1d1d; border: 1px solid #7f1d1d; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 14px;">
                            Delete Review
                        </button>
                    </form>
                @endif
    ...
    @if(isset($recommendations) && count($recommendations) > 0)
        <div style="margin-top: 48px; border-top: 1px solid #1e1e2e; padding-top: 48px;">
            <h2 style="font-family: var(--font-display); font-size: 32px; margin-bottom: 24px;">
                You Might Also Like
            </h2>
            <div style="display: flex; gap: 24px; overflow-x: auto; padding-bottom: 16px;">
                @foreach($recommendations as $rec)
                    <div style="min-width: 160px; max-width: 160px; background: #13131a; border: 1px solid #1e1e2e; border-radius: 12px; overflow: hidden;">
                        @if($rec->poster)
                            <img src="{{ $rec->poster }}" alt="{{ $rec->title }}" style="width: 100%; height: 240px; object-fit: cover;">
                        @else
                            <div style="width: 100%; height: 240px; display: flex; align-items: center; justify-content: center; color: #6b6b80; background: #0a0a0f;">
                                No Poster
                            </div>
                        @endif
                        <div style="padding: 12px; text-align: center;">
                            <p style="color: #e8e8f0; font-weight: 600; font-size: 14px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="{{ $rec->title }}">
                                {{ $rec->title }}
                            </p>
                        </div>
                    </div>
